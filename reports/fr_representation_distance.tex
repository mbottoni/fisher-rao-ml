...
FR-RD on two datasets: UCI Digits (1,797 samples, 5 conditions, 5 seeds) and MNIST
(3,000 train, 5 conditions, 5 seeds). Both yield a between-to-within separation ratio
of $\approx1.47$--$1.50\times$ and a correlation with accuracy differences of $r=0.74$,
confirming that the properties are dataset-agnostic. We further show that FR-RD between
...
and fine-tuning divergence tracking. An OOD detection experiment reveals a failure mode of
centroid-based scoring for confident classifiers, motivating class-conditional variants.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A comparison to linear CKA, highlighting that they are complementary: FR-RD
        ...
        in hidden layers.
  \item An empirical OOD detection experiment showing that centroid-based FR-RD scoring
        fails for high-confidence classifiers (centroid collapses to uniform, inverting
        the ranking), with a mechanistic explanation and a proposed class-conditional fix.
\end{enumerate}
...
300 training steps. Five seeds per condition (25 models total). FR-RD is computed on the
held-out test set ($n \approx 539$).

\textbf{MNIST}: 3,000 training examples (stratified from 60k), 500 test examples, 784
flattened pixels, 400 steps. Five seeds per condition (25 models total).

\subsection{Pairwise FR-RD separates training conditions}

Figure~\ref{fig:heatmap} shows the mean pairwise FR-RD and linear CKA across all $\binom{25}{2}
= 300$ model pairs. FR-RD cleanly separates conditions on both datasets:
\begin{itemize}
  \item CE (clean) vs.\ FR (clean): $\mathrm{FR\text{-}RD} = 0.064$ on Digits and $0.121$
  ...
  \item Within-condition FR-RD on Digits: mean $0.079$ (clean), $0.071$ (FR-loss), $0.157$
        (smoothed), $1.427$ (30\% noise), $2.230$ (60\% noise). MNIST shows the same
        pattern: $0.129$, $0.136$, $0.260$, $1.398$, $2.296$ respectively.
  \item Between-condition vs.\ within-condition ratio: $1.50\times$ on Digits and $1.47\times$
        on MNIST, demonstrating consistent systematic separation across datasets.
  \item Correlation with accuracy differences: $r = 0.74$ on both Digits and MNIST
        (vs.\ $|1 - \mathrm{CKA}| = 0.85$ on MNIST), showing that FR-RD is a reliable
        predictor of behavioral divergence.
\end{itemize}
...
\begin{table}[t]
  \centering
  \caption{Mean test accuracy $\pm$ std across 5 seeds, and mean within-condition FR-RD.}
  \label{tab:accuracy}
  \scriptsize
  \begin{tabular}{lrr}
    \toprule
    Condition & Test accuracy $\uparrow$ & Within-condition FR-RD $\downarrow$ \\
    \midrule
    CE (clean)     & $0.977 \pm 0.003$ & $0.072$ \\
    FR (clean)     & $0.977 \pm 0.003$ & $0.071$ \\
    LS (clean)     & $0.989 \pm 0.003$ & $0.156$ \\
    CE 30\% noise  & $0.733 \pm 0.010$ & $1.427$ \\
    CE 60\% noise  & $0.428 \pm 0.008$ & $2.230$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
We test a simple OOD scorer: for a trained model $\theta$ and an in-distribution (ID)
reference set, compute the centroid $\bar{p} = \frac{1}{n}\sum_i P_\theta(x_i)$ of softmax
outputs, then score a query $x$ as $s(x) = d_{\mathrm{FR}}(\bar{p}, P_\theta(x))$.
We evaluate on UCI Digits (ID) vs.\ MNIST resized to $8\times 8$ (OOD), 300 OOD samples,
5 seeds per condition.

\textbf{Result:} For well-trained clean models, the separation is \emph{negative}
(mean $-0.38$ across seeds): ID samples score \emph{higher} than OOD samples,
the opposite of the desired direction. Label-smoothed models show the strongest
negative separation ($-0.94$). Noisy models at $60\%$ noise are the exception:
4/5 seeds show positive separation (mean $+0.21$).

\textbf{Mechanistic explanation.} For a confident $C$-class classifier, the centroid
$\bar{p} \approx \frac{1}{n}\sum_i e_{y_i}$ where $e_{y_i}$ is the predicted one-hot
label. With balanced classes this centroid is near-uniform ($\approx \mathbf{1}/C$).
The ID samples themselves are near one-hot (high confidence), which are far from the
uniform centroid in Fisher--Rao distance. MNIST digits, being out-of-domain, elicit
less confident outputs --- closer to the centroid. The centroid approach therefore
\emph{inverts} the intended ranking for any high-confidence classifier.

\textbf{Implication.} Centroid FR-RD works only when the model is uncertain:
the noisy-60\% condition yields $\bar{p}$ that is already uncertain, so OOD inputs
are not closer to it. A class-conditional centroid
$\bar{p}_c = \mathbb{E}[P_\theta(x) \mid \hat{y} = c]$ and
$s(x) = \min_c d_{\mathrm{FR}}(\bar{p}_c, P_\theta(x))$ would avoid this collapse
by measuring per-class prototype distance; we leave this for future work.
...
\paragraph{OOD detection: lessons from the centroid experiment.}
The centroid-based FR-RD scorer (\S\ref{sec:ood}) fails for high-confidence models because
the population centroid collapses toward uniform, making ID samples (near one-hot) score
farther than OOD samples. This is a useful negative result: it clarifies that FR-RD is a
distance between \emph{two distributions}, not an absolute score, and that a useful OOD
score requires a meaningful reference point. Class-conditional centroids or kernel density
estimates in output space are natural remedies.
...
\begin{itemize}
  ...
  \item \textbf{Centroid OOD score inverts for confident models.} As shown in
    \S\ref{sec:ood}, the global centroid of a high-confidence classifier collapses
    toward uniform, making confident ID samples score \emph{farther} than uncertain
    OOD samples. Class-conditional centroids or learned prototypes are needed for
    reliable OOD scoring in practice.
\end{itemize}
...
FR-RD measures behavioral divergence at the output. An OOD experiment reveals an important
failure mode of global centroid scoring: the centroid of confident predictions is near-uniform,
inverting the OOD score for well-trained models; class-conditional centroids are the natural
remedy. FR-RD's metric properties and bounded scale make it a principled drop-in tool for
model comparison and fine-tuning divergence tracking, with OOD application requiring
careful choice of the reference distribution.
