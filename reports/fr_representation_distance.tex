...
We compare four OOD scorers on the same models:
...
\begin{table}[h]
  \centering
  \caption{OOD detection win counts (correct-ranking seeds out of 10) for four methods.
  FR-RD CC is the only method that achieves positive separation for both clean and noisy
  models. MSP fails for 60\%-noise models (model is uniformly uncertain);
  Mahalanobis fails for clean/FR-loss models (feature clusters are well-separated from
  OOD inputs in feature space, but feature distribution drifts confound the score).}
  \label{tab:ood}
  \small
  \begin{tabular}{lrrrr}
    \toprule
    Condition & Global FR-RD & FR-RD CC & MSP & Mahalanobis \\
    \midrule
    CE (clean)    & 0/10 & \textbf{9/10} & 9/10        & 4/10 \\
    FR (clean)    & 0/10 & \textbf{9/10} & 9/10        & 4/10 \\
    LS (clean)    & 0/10 & \textbf{10/10}& \textbf{10/10} & 10/10 \\
    CE 30\% noise & 7/10 & 7/10          & 3/10        & \textbf{10/10} \\
    CE 60\% noise & 9/10 & \textbf{10/10}& 2/10        & \textbf{10/10} \\
    \midrule
    Total         & 16/50 & \textbf{45/50} & 33/50 & 28/50 \\
    \bottomrule
  \end{tabular}
\end{table}
...
wins across all five conditions and both model quality regimes ($> $ MSP's 33/50 and
Mahalanobis's 28/50). The mechanism: $\bar{p}_c$ represents the model's expected output
...
\begin{figure}[h]
  ...
  \caption{OOD detection win counts (out of 10 seeds) for four methods across five
  training conditions. FR-RD CC is the only method that achieves $\ge 7/10$ wins for
  every condition. MSP fails for noisy models; Mahalanobis fails for clean/FR-loss models;
  global FR-RD centroid fails for all clean models.}
  \label{fig:ood}
\end{figure}
