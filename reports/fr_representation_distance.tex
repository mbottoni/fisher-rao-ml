...
Fisher information metric), making it interpretable without normalization. We demonstrate
that FR-RD separates models trained under different label-noise conditions with a
between-to-within separation ratio of $1.50\times$, tracks training dynamics in a way that
reveals convergence speed differences under noise, and correlates with accuracy differences
at $r = 0.74$ (vs.\ $|1 - \mathrm{CKA}|$ at $r = 0.85$). We further show that FR-RD
between the cross-entropy and Fisher--Rao loss objectives on clean labels is $0.064$,
confirming their behavioral equivalence under non-corrupted targets. FR-RD provides a
compute-cheap, metric-theoretically grounded tool for model comparison, fine-tuning
divergence tracking, and out-of-distribution detection.
\end{abstract}
...
\begin{enumerate}
  ...
  \item A comparison to linear CKA, highlighting that they are complementary: FR-RD
        ...
        in hidden layers.
  \item An application sketch for FR-RD-based out-of-distribution detection via the
        centroid distance in output-distribution space.
\end{enumerate}
...
$300$ steps) on the sklearn digits dataset ($1797$ examples, $64$ features, $10$ classes,
...
\end{itemize}
Five seeds are used per condition ($25$ models total). FR-RD is computed on the held-out
test set ($n \approx 539$ examples) as the reference dataset.
...
= 300$ model pairs. FR-RD cleanly separates conditions:
\begin{itemize}
  \item CE (clean) vs.\ FR (clean): $\mathrm{FR\text{-}RD} = 0.064$, confirming that the
        two objectives converge to nearly identical behavioral solutions on clean data. This
        is consistent with the second-order expansion $d_{\mathrm{FR}}^2 \approx 2\,
        \mathrm{KL}$ near the optimum.
  \item Within-condition FR-RD: mean $= 0.079$ (clean) and $0.071$ (FR-loss), versus
        $0.157$ (smoothed), $1.427$ (30\% noise), $2.230$ (60\% noise). Higher noise leads
        to higher within-condition variance --- models trained under $60\%$ noise are more
        variable than models trained under $30\%$ noise.
  \item Between-condition vs.\ within-condition ratio: $1.50\times$, demonstrating
...
\begin{figure}[t]
  ...
  \caption{Mean pairwise FR-RD (left) and linear CKA (right) across all 300 model pairs.
  FR-RD increases monotonically with noise level; clean CE and FR loss models are nearly
  indistinguishable ($0.064$). CKA shows a complementary view of hidden-feature similarity.}
  \label{fig:heatmap}
\end{figure}
...
\begin{figure}[t]
  ...
  \caption{FR-RD (left) and $1-\mathrm{CKA}$ (right) versus absolute accuracy difference
  for all $300$ model pairs. Blue: same-condition pairs; red: cross-condition pairs.
...
The FR-RD from a test input's model output to the in-distribution centroid,
$d_{\mathrm{FR}}(\bar{p}_{\mathrm{ID}}, P_\theta(x))$, provides a simple, bounded OOD
score. Unlike maximum softmax probability (MSP), this centroid distance is symmetric and
metric, enabling principled thresholding.
...
\citet{kornblith2019similarity} introduced linear CKA and showed it is more consistent
across architectures than earlier measures such as RSA \citep{kriegeskorte2008representational}.
\citet{raghu2017svcca} proposed SVCCA based on canonical correlations. All of these
operate on hidden representations and are not metrics in the strict sense.

\paragraph{Behavioral metrics.}
\citet{jiang2019fantastic} study predictors of generalization using a mix of geometric
and behavioral measures. Recent work argues for reliability audits of
CKA on finite samples. Our work takes the behavioral angle further by grounding the metric in information
geometry.

\paragraph{Fisher--Rao distance in ML.}
The categorical Fisher--Rao distance is studied in
\citet{rao1945information,nielsen2020elementary,nielsen2023fishrao}. Its use as a training
objective for t-SNE and soft-label classification is studied in the companion paper
\citep{bottoni2026fisherrao}. This paper proposes its first use as a \emph{model comparison
metric} rather than a training objective.

\section{Conclusion}

FR-RD is a proper metric on model behaviors, bounded by $\pi$, and compute-cheap. On
digits classifiers, it separates training conditions with $1.50\times$ between-to-within
ratio, tracks training dynamics, and correlates with accuracy differences at $r = 0.74$.
It is complementary to CKA: where CKA measures hidden-representation alignment, FR-RD
measures behavioral divergence at the output. Its metric properties and bounded scale make
it a principled drop-in tool for model comparison, fine-tuning divergence tracking, and
OOD scoring in any setting where model output distributions are accessible.
