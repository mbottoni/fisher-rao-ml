...
\begin{abstract}
We propose the \emph{Fisher--Rao Representation Distance} (FR-RD), the expected
Fisher--Rao geodesic distance between two models' softmax outputs over a reference
dataset: $\mathrm{FR\text{-}RD}(\theta,\phi;X) = \mathbb{E}_{x\sim
X}[d_\mathrm{FR}(P_\theta(x), P_\phi(x))]$.
Unlike CKA \citep{kornblith2019similarity}, FR-RD is a \emph{proper metric}
(triangle inequality holds; zero iff identical behavior), bounded by $\pi$, and
operates at the output layer rather than in hidden-layer representations.

The most compelling application is \emph{fine-tuning divergence tracking}: FR-RD
to a fully-trained reference model correlates with the accuracy gap across data
fractions at $r=0.963$ ($p<0.0001$, 25 pairs), providing a practical proxy for
generalization gap \emph{without requiring held-out labels}. This makes FR-RD
useful for monitoring fine-tuning runs, selecting checkpoints, and detecting
representation drift in production.

For OOD detection, a class-conditional FR-RD centroid achieves 45/50 wins across
training conditions vs.\ MSP (33/50) and Mahalanobis (38/50) — and, crucially, is
the \emph{only method that works for both clean and noisy models}: MSP fails when
models are noise-trained (low confidence everywhere), Mahalanobis fails for clean
models (feature clusters too tight), while FR-RD CC handles both.

We validate on UCI Digits and MNIST (10 seeds each), finding a between-to-within
separation ratio of $1.47$--$1.49\times$ and accuracy correlation $r=0.740$--$0.741$,
stable across datasets and confirming dataset-agnostic properties.
\end{abstract}
...
\begin{enumerate}
  \item \textbf{Fine-tuning divergence tracking.}
        FR-RD to a fully-trained reference model correlates with accuracy gap across data
        fractions at $r=0.963$ ($p<0.0001$, 25 pairs, 5 seeds), providing a held-out-label-free
        proxy for generalization gap during fine-tuning
        (Table~\ref{tab:finetuning}, Figure~\ref{fig:finetuning}).
  \item \textbf{OOD detection robust to model quality.}
        A class-conditional FR-RD centroid achieves 45/50 wins vs.\ MSP (33/50),
        Mahalanobis (38/50), and Energy (36/50) across five training conditions
        (Table~\ref{tab:ood}). Unlike all baselines, FR-RD CC works for both confident
        clean models and noisy models where MSP/Mahalanobis each fail on one regime.
  \item \textbf{Formal metric properties.}
        Proofs that FR-RD is a proper pseudometric (triangle inequality, zero iff identical
        behavior), bounded by $\pi$, invariant to label permutation within a class.
        Between-to-within separation ratio $1.47$--$1.49\times$, accuracy correlation
        $r=0.740$--$0.741$, stable across UCI Digits and MNIST (10 seeds each).
  \item \textbf{Complementarity with CKA.}
        FR-RD measures behavioral divergence at the output distribution; CKA measures
        representational geometry in hidden layers. They are complementary: FR-RD
        captures objective-induced behavioral differences invisible to CKA
        ($d_\mathrm{FR\text{-}RD}(\mathrm{CE},\mathrm{FR\text{-}loss}) = 0.070$--$0.125$
        on clean data, confirmed non-zero despite identical accuracy).
\end{enumerate}
...
\begin{figure}[t]
  ...
  1,225 model pairs on UCI Digits (top) and MNIST (bottom, 10 seeds each). FR-RD increases
  monotonically with noise level on both datasets; clean CE and FR-loss models are nearly
  indistinguishable ($0.070$/$0.125$ on Digits/MNIST). CKA shows a complementary view of
  hidden-feature similarity. The near-identical structure on both datasets confirms
  dataset-agnostic properties of FR-RD.}
  \label{fig:heatmap}
\end{figure}
...
difference $|\mathrm{acc}(\theta) - \mathrm{acc}(\phi)|$ for all 1,225 pairs. Pearson
...
\begin{figure}[t]
  ...
  \caption{FR-RD and $1-\mathrm{CKA}$ versus absolute accuracy difference for all 1,225
  model pairs per dataset (10 seeds × 5 conditions = 50 models), on UCI Digits (top) and
  MNIST (bottom). Blue: same-condition pairs; red: cross-condition pairs. Both metrics
  correlate with accuracy differences; $r=0.74$ for FR-RD on both datasets,
  $r=0.85$--$0.87$ for $1{-}\mathrm{CKA}$. FR-RD measures output-distribution divergence;
  CKA measures hidden-layer alignment.}
  \label{fig:scatter}
\end{figure}
...
question is more directly relevant. The non-zero FR-RD between CE and FR-loss models despite
identical accuracy ($d_\mathrm{FR\text{-}RD} = 0.070$--$0.125$) shows that FR-RD captures
objective-induced behavioral differences invisible to CKA and accuracy metrics: the two
models have learned different probability geometries that happen to agree on the top-1 class.

\paragraph{Why fine-tuning divergence works without labels.}
The $r = 0.963$ correlation between FR-RD and accuracy gap (Table~\ref{tab:finetuning})
arises because both measure the same underlying quantity from different directions: the
amount of information from the training data that has been incorporated into the model.
Weight-space $L_2$ distance fails because it is confounded by reparameterization (two models
may be close in weight space but far in behavior after applying a nonlinear transformation).
FR-RD is invariant to any smooth reparameterization of the network since it depends only on
output distributions. Practically, this means FR-RD to a fully-trained reference model can
serve as a \emph{convergence monitor} during fine-tuning: when FR-RD stops decreasing, the
model has absorbed the information the training data can provide, independent of the
accuracy on a held-out set.

\paragraph{Why class-conditional centroids handle both clean and noisy models.}
The OOD experiment reveals why MSP, Mahalanobis, and the global centroid each fail on one
regime: they depend on a single model property (confidence level, feature-space geometry, or
average output) that deteriorates under label noise.
FR-RD CC uses the model's \emph{class-specific} expected output $\bar{p}_c$, which remains
meaningful even when the global confidence degrades: noisy models still learn to distinguish
classes, just with less certainty.
This is the key insight: OOD detection should exploit what the model \emph{has} learned
(class-conditional structure), not what it lacks (overall confidence).
Mahalanobis distance has the analogous advantage for feature geometry but fails for clean
models with tight within-class clusters, where the shared Gaussian assumption
underestimates the distance from OOD inputs that land in low-density regions.

\paragraph{Future directions.}
\begin{itemize}
  \item \textbf{Scale to ResNet/CIFAR.} Validating FR-RD on larger models (ResNet-18 on
        CIFAR-10/100) is the most important next step. The between/within ratio and
        accuracy correlation ($r \approx 0.74$) should be confirmed at scale before
        recommending FR-RD for practical use.
  \item \textbf{Fine-tuning divergence on MNIST.} The data-fraction experiment uses UCI
        Digits only; repeating on MNIST with the same protocol would confirm dataset-agnostic
        stability of the $r = 0.963$ result.
  \item \textbf{Continual learning forgetting.} FR-RD between the original-task model and
        the model after each new task provides a label-free catastrophic forgetting monitor,
        a natural application of the fine-tuning-divergence result.
\end{itemize}
...
We proposed FR-RD, the expected Fisher--Rao geodesic distance between two models' softmax
outputs over a reference dataset. Its most compelling property is \emph{practical}: FR-RD
to a fully-trained reference model correlates with the accuracy gap across data fractions
at $r = 0.963$ ($p < 10^{-4}$, 25 pairs), providing a held-out-label-free proxy for
generalization gap during fine-tuning and a label-free convergence monitor.

For OOD detection, the class-conditional FR-RD centroid achieves 45/50 wins across five
training conditions vs.\ MSP (33/50), Mahalanobis (38/50), and Energy (36/50) — and is
the \emph{only} method that works for both confident clean models and noise-trained models:
MSP and Energy fail for noisy models (low confidence everywhere), while Mahalanobis
fails for clean models (tight feature clusters).

Formally, FR-RD is a proper pseudometric (triangle inequality, bounded by $\pi$, zero iff
identical behavior), complementary to CKA: where CKA measures hidden-representation
alignment, FR-RD measures behavioral divergence at the output. Between-to-within separation
ratios of $1.47$--$1.49\times$ and accuracy correlations $r = 0.74$ are stable across UCI
Digits and MNIST (10 seeds each), confirming dataset-agnostic properties.

The key open question is scale: validating FR-RD on ResNet models and CIFAR-scale datasets
is needed before recommending it as a general-purpose tool.
